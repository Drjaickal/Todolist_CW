...
def list_update():
    ...
    for task in tasks:
        #using the insert() method to insert the task in the list box
...
def delete_task():
    ...
    try:
        #getting the slected entry from the list box
        ...
        if the_value in tasks:
            ...
            #calling the function to update the list
            list_update()
            #usinng the execute()method to execute a SQL statement
            the_cursor.execute('delete from tasks where title=?',(the_value,))
    except:
        #displaying the message box with 'No item Selected' message fro and execption
        messagebox.showninfo('Error','No Task Selected.Cannot Delete.')
        
#function to delete all tasks from the list
def delete_all_tasks():
    #displaying a messagebox to ask user for confirmation
    message_box=messagebox.askyesno('Delete All','Are you sure?')
    #if the valuse to be True
    if message_box==True:
        ...
        while(len(tasks)!=0):
            ...
            tasks.pop()
         #using the execute() methid to execute a SQL statement
...
def close():
    #printing the elements from the task list
    print(tasks)
...
def retrieve_database():
    #using the while loop iterate through the elements in the tasks list
    while(len(tasks)!=0):
        #using the pop() method to pop out the elements from the list
        tasks.pop()
    #iterating through the rows in the databse table
...
if __name__ == "__main__":  
    ...
    #setting the geometry tot the window
...
#using the connect() method to the database
...
#using the pack() method to place in the application
...
#using the place() method to place the entry in the application
task_field.place(x=30,y=80)

#adding the buttons to the application using the ttk.button() widget
...
delete_all_button=ttk.Button(functions_frame,text="Delete All Tasks",width=24,command=delete_all_tasks)
...
#defining a list box the tk.Listbox() widget
...
guiWindow.mainloop()
#establising the connection with database
